...
9. Implemented Phase 4 shared read-path cutover: [rootfs/usr/share/planefence/pf-db.py](rootfs/usr/share/planefence/pf-db.py) now includes `load-day-state` to materialize bash maps from sqlite row tables (`pf_records`, `pa_records`, `heatmap`, `globals_kv`), and [rootfs/scripts/pf-common](rootfs/scripts/pf-common) `READ_RECORDS()` in sqlite mode now prefers this row-table state path with snapshot fallback for compatibility.
