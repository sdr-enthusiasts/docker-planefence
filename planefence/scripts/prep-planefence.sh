...
[[ "x$PF_ALERTLIST" != "x" ]] && sed -i 's|\(^\s*ALERTLIST=\).*|\1'"$PF_ALERTLIST"'|' /usr/share/plane-alert/plane-alert.conf || sed -i 's|\(^\s*ALERTLIST=\).*|\1|' /usr/share/plane-alert/plane-alert.conf
