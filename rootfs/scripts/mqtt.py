...
def main():
    ...
    # Publish the message
    try:
        publish.single(topic=args.topic, payload=args.message, qos=args.qos, retain=True, hostname=args.broker, port=args.port, client_id=args.client_id, **({"auth": {'username': args.username, 'password': args.password}} if args.username and args.password else {}), **({"tls": ssl.create_default_context()} if enable_tls else {}))
