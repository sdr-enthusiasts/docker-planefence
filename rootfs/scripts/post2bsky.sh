...
if (( ${#cid[@]} == 0 )); then
    ...
    if (( ${#hashtags[@]} + ${#urlstart[@]} > 0 )); then
        ...
        if (( ${#urlstart[@]} > 0 )); then
            for url in "${!urlstart[@]}"; do
                shortlink="$(curl -G --data-urlencode "url=${urluri[${url}]}" "https://is.gd/create.php?format=simple")"
                if [[ -n "$shortlink" ]]; then urluri["$url"]="$shortlink"; fi
                post_data+="
                    {
                        \"index\": {
                            \"byteStart\": ${urlstart[${url}]},
...
if [[ "$(jq -r '.uri' <<< "$response")" != "null" ]]; then
        ...
        log_print INFO "BlueSky Post successful. Post available at https://bsky.app/profile/$handle/post/${uri##*/}. $ratelimit_str"
else
