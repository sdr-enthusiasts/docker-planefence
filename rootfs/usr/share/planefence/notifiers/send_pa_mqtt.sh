...
generate_mqtt() {
  # Generate a MQTT notification
	...
	if [[ -n "$MQTT_URL" ]]; then
		# convert $pa_records[@] into a JSON object; if (PF_)MQTT_FIELDS is defined, then only use those fields. Exclude any internal stuff
		declare -a keys=()
...
for idx in "${!link[@]}"; do
    pa_records["$idx":mqtt:notified]="${link[idx]}"
done
