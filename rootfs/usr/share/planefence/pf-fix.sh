...
do
	...
	then
		...
		if [[ "${r[1]#@}" != "" ]] && [[ "$LOGLEVEL" != "ERROR" ]]; then "${s6wrap[@]}" echo " ${r[0]} = ${r[1]#@} (from socket30003 data)"; fi
	fi
	...
	then
		...
		if [[ "${r[1]#@}" != "" ]] && [[ "$LOGLEVEL" != "ERROR" ]]; then "${s6wrap[@]}"  echo " ${r[0]} = ${r[1]#@} (from mictronic database cache)"; fi
	fi
	...
	if [[ -z "${r[1]#@}" ]] && [[ "${r[0]:0:1}" == "A" ]]; then
		...
		if [[ -n "${r[1]#@}" ]] && [[ "$LOGLEVEL" != "ERROR" ]]; then "${s6wrap[@]}" && echo "  ${r[0]} = ${r[1]#@} (from US Hex ID)"; fi
	fi
