...
- The heatmap shows where planes come very often. It makes common routes visible.
...
from a dump1090 host using port 30003 and calculates the distance and
angle between the antenna and the plane. It will store these values in an
output file in csv format (separated by commas) together with other flight
data.
...
The script can be launched as a background process. It can be stopped by
...
Notes:
        - To launch it as a background process, add '&' or run it from crontab:
          ...
          (This command checks if it ran every hour and relauch it if necessary.)
...
The script will automatically use the correct units (feet, meter,
...
antenna. You can change the default range by specifying the number
of degrees -/+ to your antenna locations. (The default values will
probably satisfy.) This area will be divided in to small squares.
...
too much positions in the heatmap. Of course this also depends on
...
The best thing is to leave it as much as it is. May change some directories of the unit types (meters, kilometers, miles, feet, nautical miles etc).
...
Files will be copied to the install directory.
...
Of course you can run 'socket30003.pl' from the commandline, but if you want to leave it running for days or weeks you should add it as a crontab job.
...
Copy the 'rangview.kml' to a webserver where it can be publicly accessed (necessary for the Google maps API).
